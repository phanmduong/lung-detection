from resize_image import resize_3d, resize_multilinear_tf
...
class RoiPoolingConv(Layer):
    ...
    def call(self, x, mask=None):
        ...
        for roi_idx in range(self.num_rois):
            ...
            deep_length = d / float(self.pool_size)
            ...
            h = K.cast(h, 'int32')
            d = K.cast(d, 'int32')
            print(img[:, y:y+h, x:x+w,z:z+d, :])
            # rs = tf.image.resize(img[:, y:y+h, x:x+w,z:z+d, :], (self.pool_size, self.pool_size, self.pool_size))
            # rs = LinearResizeLayer((self.pool_size, self.pool_size, self.pool_size))(img[:, y:y+h, x:x+w,z:z+d, :])
            rs = resize_multilinear_tf(img[:, y:y+h, x:x+w,z:z+d, :], K.constant([self.pool_size, self.pool_size, self.pool_size]))
            outputs.append(rs)
